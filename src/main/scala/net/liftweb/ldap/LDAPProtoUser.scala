...
import net.liftweb.mapper.{MappedString, MegaProtoUser, MetaMegaProtoUser}
...
trait MetaLDAPProtoUser[ModelType <: LDAPProtoUser[ModelType]] extends MetaMegaProtoUser[ModelType] {
  ...
  override def loginXhtml : Elem = {
    val loginStr = S.?("log.in")

    <form method="post" action={S.uri}>
      <table>
        <tr>
          <td colspan="2">{loginStr}</td>
        </tr>
        <tr>
        ...
        <tr>
          <td>&nbsp;</td><td><input type="submit" value={loginStr}/></td>
        </tr>
      </table>
    </form>
  }
  ...
  override def login : NodeSeq = {
    ...
    def bind: CssSel = {
      val passwordElemId = nextFuncName
      val usernameElemId = nextFuncName

      S.appendJs(JsCmds.Focus(passwordElemId) & JsCmds.Focus(usernameElemId))

      "type=text [id]" #> usernameElemId &
      "type=password [id]" #> passwordElemId
    }
